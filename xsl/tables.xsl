...
>
  ...
  <xsl:template match="*[contains(@class, ' topic/table ')]">
    <ast:node type="table">
      <ast:props>
        <!-- data- prefix: DITA concepts, not real table attributes -->
        <xsl:if test="@frame">
          <ast:prop name="data-frame" value="{@frame}"/>
        </xsl:if>
        <xsl:if test="@pgwide">
          <ast:prop name="data-pgwide" type="boolean" value="{if (@pgwide = 'yes') then 'true' else 'false'}"/>
        </xsl:if>
        <xsl:if test="@color">
          <ast:prop name="data-variant" value="{@color}"/>
        </xsl:if>
  ...
  <xsl:template match="*[contains(@class, ' topic/entry ')]">
    ...
    <ast:node type="{if ($header) then 'th' else 'td'}">
      <ast:props>
        ...
        <xsl:variable name="color" select="(@color, ancestor::*[contains(@class, ' topic/table ')][1]/@color)[1]"/>
        <xsl:if test="$color">
          <ast:prop name="data-variant" value="{$color}"/>
        </xsl:if>
        <xsl:call-template name="common-props"/>
