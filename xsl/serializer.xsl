...
>
  ...
  <!-- one ast:prop-array -> one fn:array of strings, for props like Accordion's defaultActiveKey -->
  ...
  <!-- per-topic document envelope: {"meta": {...}, "content": [tuples]} -->
  <xsl:function name="ast:serialize-document" as="xs:string">
    ...
    <xsl:variable name="tree" as="element(fn:map)">
      <fn:map>
        <fn:map key="meta">
          <xsl:apply-templates select="$meta/ast:prop" mode="to-fn-json-prop"/>
        </fn:map>
  ...
  <!-- merged TOC document envelope: {"toc": [tuples]} -->
